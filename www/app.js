...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMenu() {
        ...
        cafeteriaMenu.innerHTML = menuItems.map(item => `
            <div class="bg-stone-50 p-6 rounded-[2.5rem] border border-stone-100 hover:border-sano-300 transition-all group cursor-pointer shadow-sm hover:shadow-xl">
                <div class="flex justify-between items-start mb-6">
                    ...
                    <span class="text-wine-700 font-black text-xl">${item.price}</span>
                </div>
                <h4 class="text-xl font-bold mb-2">${item.name}</h4>
                <div class="flex items-center gap-2 mb-6">
                    <span class="text-xs font-black text-stone-400 uppercase tracking-tighter">${item.calories} kcal</span>
                    <div class="flex gap-1">
                        ${item.tags.map(tag => `<span class="text-[10px] bg-sano-50 text-sano-700 px-2 py-0.5 rounded-full font-bold">${tag}</span>`).join('')}
                    </div>
                </div>
                <button class="w-full py-3 bg-white border-2 border-stone-100 rounded-xl font-bold text-sm hover:bg-sano-600 hover:text-white hover:border-sano-600 transition-all">
                    Registrar Consumo
                </button>
            </div>
        `).join('');
    ...
    function showToast(message, type = 'success') {
        ...
        toast.className = `flex items-center gap-3 p-4 rounded-2xl shadow-xl border bg-white animate-slide-up pointer-events-auto min-w-[300px] z-[70]`;
        
        const icon = type === 'success' ? 'check-circle' : 'alert-circle';
        const iconColor = type === 'success' ? 'text-emerald-500' : 'text-rose-500';
        
        toast.innerHTML = `
            <div class="p-2 ${type === 'success' ? 'bg-emerald-50' : 'bg-rose-50'} rounded-xl">
                <i data-lucide="${icon}" class="w-5 h-5 ${iconColor}"></i>
            </div>
            <div class="flex-1">
                <p class="text-sm font-bold text-stone-900">${message}</p>
            </div>
            <button class="p-1 hover:bg-stone-50 rounded-lg text-stone-400">
    ...
    function updateRecentSearchesUI() {
        ...
        recentSearchesList.innerHTML = recentSearches.map(term => `
            <button class="px-4 py-2 bg-stone-100 rounded-full text-xs font-bold text-stone-500 hover:bg-sano-50 hover:text-sano-700 transition-all recent-item">
                ${term}
            </button>
        `).join('');
    ...
    function displayModernResult(food) {
        modernAnalysisResult.innerHTML = `
            <div class="bg-white rounded-[3rem] p-8 border border-stone-100 shadow-2xl overflow-hidden relative">
                <div class="flex flex-col md:flex-row gap-10">
                    <div class="flex-1">
                        <div class="flex items-center gap-4 mb-6">
                            <div class="w-16 h-16 bg-sano-100 rounded-3xl flex items-center justify-center text-3xl">
                                ${food.category === 'tacos' ? '🌮' : (food.category === 'abarrotes' ? '📦' : '🍲')}
                            </div>
                            <div>
                                <h3 class="text-3xl font-black text-stone-900">${food.label}</h3>
                                <div class="flex gap-2 mt-1">
                                    ${food.tags.map(t => `<span class="text-[10px] font-black uppercase tracking-tighter text-sano-600 bg-sano-50 px-2 py-0.5 rounded-full">${t}</span>`).join('')}
                                </div>
                            </div>
                        </div>
                        <p class="text-stone-500 leading-relaxed font-medium mb-8">${food.description}</p>
                        
                        ${food.price ? `
                            <div class="mb-6 p-4 bg-stone-50 rounded-2xl border border-dashed border-stone-200">
                                <div class="flex justify-between items-center mb-2">
                                    <span class="text-xs font-black text-stone-400 uppercase">Precio Estimado</span>
                                    <span class="text-sm font-black text-wine-700">${food.price}</span>
                                </div>
                                ${food.substitutes ? `
                                    <div class="flex gap-2 items-center">
                                        <span class="text-[10px] font-black text-stone-400 uppercase">Sustitutos:</span>
                                        <div class="flex gap-1">
                                            ${food.substitutes.map(s => `<span class="text-[10px] font-bold text-stone-500 bg-white px-2 py-0.5 rounded-lg border border-stone-100">${s}</span>`).join('')}
                                        </div>
                                    </div>
                                ` : ''}
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="p-6 bg-stone-50 rounded-[2rem] border border-stone-100">
                                <p class="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1">Calorías</p>
                                <p class="text-3xl font-black text-wine-900">${food.calories} <span class="text-sm font-bold text-stone-400">kcal</span></p>
                            </div>
                            <div class="p-6 bg-stone-50 rounded-[2rem] border border-stone-100">
                                <p class="text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1">Proteína</p>
                                <p class="text-3xl font-black text-emerald-600">${food.protein} <span class="text-sm font-bold text-stone-400">g</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="w-full md:w-72 space-y-4">
                        <div class="p-6 bg-white border border-stone-100 rounded-3xl shadow-sm">
                            <div class="flex justify-between items-center mb-4">
                                <span class="text-xs font-black text-stone-400 uppercase">Grasas</span>
                                <span class="text-sm font-black text-rose-500">${food.fat}g</span>
                            </div>
                        ...
                        <div class="p-6 bg-white border border-stone-100 rounded-3xl shadow-sm">
                            <div class="flex justify-between items-center mb-4">
                                <span class="text-xs font-black text-stone-400 uppercase">Carbohidratos</span>
                                <span class="text-sm font-black text-amber-500">${food.carbs}g</span>
                            </div>
                            <div class="h-2 bg-stone-100 rounded-full overflow-hidden">
                                <div class="h-full bg-amber-500" style="width: ${Math.min(food.carbs, 100)}%"></div>
                            </div>
                        </div>
                        <button class="w-full py-4 bg-sano-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-sano-100 hover:bg-sano-700 transition-all flex items-center justify-center gap-2">
    ...
    foodSearchModern.addEventListener('input', (e) => {
        ...
        if (matches.length > 0) {
            searchSuggestions.innerHTML = matches.map(m => `
                <div class="px-6 py-4 hover:bg-stone-50 cursor-pointer border-b border-stone-50 last:border-0 flex items-center gap-3 suggestion-item">
                    <i data-lucide="search" class="w-4 h-4 text-stone-300"></i>
                    <span class="font-bold text-stone-600">${mexicanFoodDB[m].label}</span>
                </div>
            `).join('');
    ...
    function addMessage(text, isBot = true) {
        ...
        msg.innerHTML = `
            <div class="w-8 h-8 ${isBot ? 'bg-wine-100' : 'bg-sano-100'} rounded-full flex items-center justify-center flex-shrink-0">
                <i data-lucide="${isBot ? 'bot' : 'user'}" class="w-4 h-4 ${isBot ? 'text-wine-700' : 'text-sano-700'}"></i>
            </div>
            <div class="${isBot ? 'bg-white' : 'bg-wine-700 text-white'} p-3 rounded-2xl ${isBot ? 'rounded-tl-none' : 'rounded-tr-none'} shadow-sm text-sm max-w-[80%]">
                ${text}
            </div>
        `;
