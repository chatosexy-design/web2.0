...
export const getDishes = async (req: Request, res: Response, next: NextFunction) => {
  try {
    let dishes = await Dish.find({ available: true })
      .sort({ category: 1, name: 1 });

    if (!dishes || dishes.length === 0) {
      dishes = await Dish.insertMany(FULL_MENU);
    }
...
export const updateDish = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!dish) return res.status(404).json({ success: false, error: 'Platillo no encontrado' });
