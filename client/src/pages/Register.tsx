...
import { UserPlus, Mail, Lock, CreditCard, ArrowRight } from 'lucide-react';
import api from '../api';
...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post('/auth/register', { email, password, controlNumber });
      if (res.data.success) {
  ...
  return (
    <div className="max-w-md mx-auto animate-slide-up">
      <div className="card-premium p-12 text-center">
        ...
        <p className="text-stone-400 font-bold uppercase tracking-widest text-[10px] mb-10">Usa tu número de control oficial</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="text-left">
            <label className="text-xs font-black text-stone-400 uppercase tracking-widest mb-3 block ml-2">Número de Control</label>
          ...
          >
            {loading ? 'Validando...' : <>Crear Cuenta <ArrowRight className="w-5 h-5" /></>}
          </button>
        </form>
