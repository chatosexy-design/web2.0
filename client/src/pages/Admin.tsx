    ...
    <div className="max-w-7xl mx-auto space-y-12 animate-slide-up">
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        ...
        <div className="card-premium p-10">
          ...
          <div className="space-y-6">
            {Object.entries(stats.groupAverages).map(([group, data]: [string, any], i) => (
              <div key={i} className="space-y-3">
                <div className="flex justify-between text-sm font-black text-stone-400 uppercase tracking-widest">
                  <span>Grupo {group}</span>
                  <span className="text-wine-700">{(data.total / (data.count || 1)).toFixed(0)} kcal avg</span>
                </div>
                <div className="h-3 bg-stone-100 rounded-full overflow-hidden dark:bg-stone-900">
                  <div className="h-full bg-wine-700" style={{ width: `${Math.min((data.total / (data.count || 1)) / 25, 100)}%` }}></div>
                </div>
              </div>
            ))}
          </div>
